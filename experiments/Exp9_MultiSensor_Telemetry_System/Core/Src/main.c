...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  char start_msg[] = "\r===== SENSOR SYSTEM START =====\r";
   HAL_UART_Transmit(&huart1, (uint8_t*)start_msg, strlen(start_msg), 1000);

   // HTS221 (Temp + Humidity)
   if (BSP_HSENSOR_Init() == HSENSOR_OK && BSP_TSENSOR_Init() == TSENSOR_OK)
   {
       char msg[] = "HTS221 OK\r";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }
   else
   {
       char msg[] = "HTS221 ERROR";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }
   ...
   {
       char msg[] = "LPS22HB OK\r";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }
   else
   {
       char msg[] = "Pressure ERROR\r";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }

   if (BSP_ACCELERO_Init() == ACCELERO_OK)
   {
       char msg[] = "LSM6DSL Accelerometer OK\r";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }
   else
   {
       char msg[] = "Accelerometer ERROR\r";
       HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), 1000);
   }
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  // Read environmental sensors
	    ...
	    // Magnitude (useful for motion detection)
	    float a_mag = sqrt(ax*ax + ay*ay + az*az);

	    // Format UART output
	    sprintf(uart_buf,
	        "Temp: %.2f C | Hum: %.2f %% | Pressure: %.2f hPa | Accel:=%.2f g\r\n",
	        temp, hum, pressure,
	        ax, ay, az, a_mag
	    );
